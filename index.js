...
async function scoutTrends() {
  ...
  try {
    ...
    const prompt = 'You are the SCOUT. Research top 15 Bitcoin/crypto trends from internet, X, social media. PRIORITY: Research what these influential accounts are posting about TODAY: @CryptoRover, @MrCryptoWhale, @WatcherGuru, @whale_guru, @whale_insider, @trendingbitcoin. Find their latest themes, hot takes, and viral posts. Focus on GLOBAL crypto: Bitcoin price action, whale movements, ETF flows, DeFi, Layer 2s, macro news, mining. Include ONLY 30% India-related topics (regulations, exchanges, influencers). Current BTC: $' + btcStr + avoidClause + '. Return JSON: [{"topic":"name","who":"source","statement":"what they said","impact":"why it matters"}]. Mix: 70% global, 30% India. Only JSON.';
...
async function architectWork(trends) {
  ...
  try {
    ...
    const prompt = 'You are the ARCHITECT. Create 25 viral X posts from these topics: ' + JSON.stringify(trends) + '. BTC: $' + btcStr + '. These topics are inspired by @CryptoRover, @MrCryptoWhale, @WatcherGuru, @whale_guru, @whale_insider and global sources. REQUIREMENTS: 1) ONLY 30% India-focused posts (Indian regulations, exchanges, influencers). The other 70% should be GLOBAL crypto topics: Bitcoin price, whale movements, ETF flows, DeFi, Layer 2s, macro, mining. 2) Create ORIGINAL takes - NOT copies of their posts 3) Mix content types: - 5-6 SHORT posts (200-280 chars, conversational, punchy, NO hashtags, NO emojis) - 5-6 LONG posts (500-600 chars, detailed analysis, WITH hashtags + emojis) - 5-6 HUMOROUS posts (witty, relatable crypto humor, engaging) - 8-9 REGULAR posts (balanced, informative, WITH hashtags + emojis) 4) SHORT posts: pure conversational, natural style, no hashtags, no emojis 5) Other posts: use emojis and 3-5 hashtags 6) Randomize order so types are mixed. Return JSON: [{"post":"text"}]. Only JSON.';
